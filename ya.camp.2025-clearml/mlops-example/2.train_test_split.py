...
pipe = PipelineController(
    ...
    docker_args=(
        ...
        "--env CLEARML_API_ACCESS_KEY=YWC7YJ7ZTKUOLLO3C2INBYTMVDRQRV "
        "--env CLEARML_API_SECRET_KEY=ebt3LHGGI0UJhFyqLZFTb7zGtAGdf5Wi3LPjM4evRCpaEcWYIuH--mNVREM5MxLkpwQ "
...
def dataset_train_test_split(
    ...
    print("Starting dataset_train_test_split")
    ...
    print(f"Got dataset: {dataset.id}")

    datset_path = Path(dataset.get_local_copy())
    print(f"Local copy at: {datset_path}")

    files = list(datset_path.iterdir())
    print(f"Found {len(files)} files: {files[:3]}")

    print("Reading CSVs with polars...")
    dfs = []
    for i, f in enumerate(datset_path.iterdir()):
        print(f"  → Reading {f.name} ({i+1}/{len(files)})")
        df = pl.read_csv(f)
